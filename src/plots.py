...
def create_optimization_plot(df: pd.DataFrame, target_col: str = "Isp (s)") -> Figure:
    ...
    # Plot the 3D surface
    X, Y = np.meshgrid(unique_ofs, unique_pcs)
    Z = np.zeros_like(X)
    
    # Fill the Z values from the dataframe
    for i, pc in enumerate(unique_pcs):
        for j, of in enumerate(unique_ofs):
            subset = df[(df["Pc (bar)"] == pc) & (df["O/F"] == of)]
            if not subset.empty:
                Z[i, j] = subset[target_col].values[0]
    
    surf = ax.plot_surface(X, Y, Z, cmap='viridis', alpha=0.8, 
                          linewidth=0, antialiased=True)
    
    # Add a color bar
    fig.colorbar(surf, ax=ax, shrink=0.5, aspect=5, label=target_col)
    
    # Find the maximum point
    max_idx = np.unravel_index(np.argmax(Z), Z.shape)
    max_of, max_pc, max_val = X[max_idx], Y[max_idx], Z[max_idx]
    
    # Mark the maximum point
    ax.scatter([max_of], [max_pc], [max_val], color='red', s=100, marker='*')
    ax.text(max_of, max_pc, max_val, f"Maximum: {max_val:.2f}\nO/F={max_of:.2f}, Pc={max_pc:.2f} bar", 
